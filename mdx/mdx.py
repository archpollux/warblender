...
class SEQS(MDXBlock):
	...
	def loadBlock(self, file):
		...
		self.nbytes = readLong(file)
		self.seqs = []
		for self.nseqs in range(0, self.nbytes / SEQ_SIZE):
			seq = Struct()
...
class KGSC_Base(MDXBlock):
	...
	def loadBlock(self, file):
		...
		for i in range(self.nunks):
			scale = Struct()
			scale.Frame = readLong(file)
			scale.x = readFloat(file)
			scale.y = readFloat(file)
			scale.z = readFloat(file)
			if self.LineType > 1:
				scale.InTanx = readFloat(file)
				scale.InTany = readFloat(file)
				scale.InTanz = readFloat(file)
				scale.OutTanx = readFloat(file)
				scale.OutTany = readFloat(file)
				scale.OutTanz = readFloat(file)
	...
	def toString(self):
		...
		for i in range(self.nunks):
			...
			s += ".scaling[%d].Frame = %d\n" % (i, scale.Frame)
			s += ".scaling[%d].(x, y, z) = (%f, %f, %f)\n" % (i, scale.x, scale.y, scale.z)
			if self.LineType > 1:
				s += ".scaling[%d].InTax:(%f, %f, %f)\n" % (i, scale.InTanx, scale.InTaxy, scale.InTanz)
				s += ".scaling[%d].OutTan:(%f, %f, %f)\n" % (i, scale.OutTanx, scale.OutTaxy, scale.OutTanz)
		return s
	...
	def blockToHTML(self, parent, root):
		...
		s += html.property('GlobalSeqId', '%d', self.GlobalSeqId)
		s1 = html.startLoop()
		for scale_i in range(self.nunks):
			scale = self.scaling[scale_i]
			loop = html.startLoop()
			loop += html.property('Frame', '%d', scale.Frame)
			loop += html.property('x', '%f', scale.x)
			loop += html.property('y', '%f', scale.y)
			loop += html.property('z', '%f', scale.z)
			if self.LineType > 1:
				loop += html.property('InTanx', '%f', scale.InTanx)
				loop += html.property('InTany', '%f', scale.InTany)
				loop += html.property('InTanz', '%f', scale.InTanz)
				loop += html.property('OutTanx', '%f', scale.OutTanx)
				loop += html.property('OutTany', '%f', scale.OutTany)
				loop += html.property('OutTanz', '%f', scale.OutTanz)
			loop += html.endLoop()
			s1 += html.indexProp(scale_i, '%s', loop)
		s1 += html.endLoop()
		s += html.property('scaling', '%s', s1, block=self, hide=1)
		return s
...
class TXAN(MDXBlock):
	...
	def loadBlock(self, file):
		...
		while anims_size < self.nbytes:
			...
			anim.KTAT = KTAT(1)
			anim.KTAT.readFrom(file)
			anim.KTAR = KTAR(1)
			anim.KTAR.readFrom(file)
			anim.KTAS = KTAS(1)
			anim.KTAS.readFrom(file)
...
class GEOA(MDXBlock):
	...
	def blockToHTML(self, parent, root):
		...
		s1 += html.endLoop()
		s = html.property('geosanims', '%s', s1, block=self)
		return s
...
class LITE(MDXBlock):
	...
	def loadBlock(self, file):
		...
		while lights_size < self.nbytes:
			...
			light.KLAI = KLAI(1)
			light.KLAI.readFrom(file)
			light.KLAV = KLAV(1)
			light.KLAV.readFrom(file)
			light.KLAC = KLAC(1)
			light.KLAC.readFrom(file)
			light.KLBC = KLBC(1)
			light.KLBC.readFrom(file)
			light.KLBI = KLBI(1)
			light.KLBI.readFrom(file)
...
class PREM(MDXBlock):
	...
	def loadBlock(self, file):
		...
		while prems_size < self.nbytes:
			...
			prem.KGTR = KGTR(1)
			prem.KGTR.readFrom(file)
			prem.KGRT = KGRT(1)
			prem.KGRT.readFrom(file)
			prem.KGSC = KGSC(1)
			prem.KGSC.readFrom(file)
...
class KGRT(MDXBlock):
	...
	def blockToHTML(self, parent, root):
		...
		s += html.property('GlobalSeqId', '%d', self.GlobalSeqId)
		s1 = html.startLoop()
		for rot_i in range(self.nunks):
			rot = self.rotation[rot_i]
			loop = html.startLoop()
...
class PRE2(MDXBlock):
	...
	def loadBlock(self, file):
		...
		while prems_size < self.nbytes:
			...
			prem.KP2S = KP2S(1)
			prem.KP2S.readFrom(file)
			prem.KP2L = KP2L(1)
			prem.KP2L.readFrom(file)
			prem.KP2E = KP2E(1)
			prem.KP2E.readFrom(file)
			prem.KP2V = KP2V(1)
			prem.KP2V.readFrom(file)
			prem.KP2N = KP2N(1)
			prem.KP2N.readFrom(file)
			prem.KP2W = KP2W(1)
			prem.KP2W.readFrom(file)
...
class RIBB(MDXBlock):
	...
	def loadBlock(self, file):
		...
		while ribbs_size < self.nbytes:
			...
			ribb.KGTR = KGTR(1)
			ribb.KGTR.readFrom(file)
			ribb.KGRT = KGRT(1)
			ribb.KGRT.readFrom(file)
			ribb.KGSC = KGSC(1)
			ribb.KGSC.readFrom(file)
			...
			ribb.KRVS = KRVS(1)
			ribb.KRVS.readFrom(file)
			ribb.KRHA = KRHA(1)
			ribb.KRHA.readFrom(file)
			ribb.KRHB = KRHB(1)
			ribb.KRHB.readFrom(file)
...
class CAMS(MDXBlock):
	...
	def loadBlock(self, file):
		...
		while cams_size < self.nbytes:
			...
			cam.Target.KCTR.readFrom(file)

			cam.KCRL = KCRL(1)
			cam.KCRL.readFrom(file)
			cam.KTTR = KTTR(1)
			cam.KTTR.readFrom(file)
...
class OBJ(MDXBlock):
	...
	def loadBlock(self, file):
		...
		self.KGTR = KGTR(1)
		self.KGTR.readFrom(file)
		self.KGRT = KGRT(1)
		self.KGRT.readFrom(file)
		self.KGSC = KGSC(1)
		self.KGSC.readFrom(file)
		self.KATV = KATV(1)
		self.KATV.readFrom(file)
		self.present = 1
...
class Reader:
	...
	def load(self, file):
		model = Model()
		model.MDLX = MDLX()
		model.MDLX.readFrom(file)
		model.VERS = VERS()
		model.VERS.readFrom(file)
		model.MODL = MODL()
		model.MODL.readFrom(file)
		model.SEQS = SEQS()
		model.SEQS.readFrom(file)
		model.GLBS = GLBS(1)
		model.GLBS.readFrom(file)
		model.MTLS = MTLS()
		model.MTLS.readFrom(file)
		model.TEXS = TEXS()
		model.TEXS.readFrom(file)
		model.TXAN = TXAN(1)
		model.TXAN.readFrom(file)
		model.GEOS = GEOS()
		model.GEOS.readFrom(file)
		model.GEOA = GEOA(1)
		model.GEOA.readFrom(file)
		model.BONE = BONE()
		model.BONE.readFrom(file)
		model.LITE = LITE(1)
		model.LITE.readFrom(file)
		model.HELP = HELP(1)
		model.HELP.readFrom(file)
		model.ATCH = ATCH(1)
		model.ATCH.readFrom(file)
		model.PIVT = PIVT()
		model.PIVT.readFrom(file)
		model.PREM = PREM(1)
		model.PREM.readFrom(file)
		model.PRE2 = PRE2(1)
		model.PRE2.readFrom(file)
		model.RIBB = RIBB(1)
		model.RIBB.readFrom(file)
		model.CAMS = CAMS(1)
		model.CAMS.readFrom(file)
		model.EVTS = EVTS(1)
		model.EVTS.readFrom(file)
		model.CLID = CLID(1)
		model.CLID.readFrom(file)
		return model
